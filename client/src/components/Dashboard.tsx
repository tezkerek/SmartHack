...
function Dashboard({ pass, token }) {
  ...
  const [passwordGeneratorConfig, setPasswordGeneratorConfig] =
  ...
  useEffect(() => {
    const getEntries = async () => {
      if (!sessionStorage.getItem("token")) return (window.location.href = "/");
      ...
      if (result.status === "error") {
        ...
        setPasswordEntries(
          result.data.passwordEntries.map((item) => {
            item.password = decrypt(
              item.password,
              sessionStorage.getItem("crpt"),
              result.data.cryptoSalt
            );
            return item;
          })
        );
        setKeyEntries(
          result.data.keyEntries.map((item) => {
            item.password = decrypt(
              item.password,
              sessionStorage.getItem("crpt"),
              result.data.cryptoSalt
  ...
  const handleChange = (e) => {
    ...
    else setNewPassword({ ...newPassword, [name]: [target.checked] });
  };
  const handleChecks = (e: React.ChangeEvent<HTMLInputElement>) => {
    setPasswordGeneratorConfig({
  ...
  const keyHandleChange = (e) => {
    ...
    else setNewKey({ ...newKey, [name]: target.value });
  };
  ...
  const passwordsContent = (
    <>
      ...
      <form className="popup-form">
        ...
        <button className="popup-btn" type="button" onClick={handleSubmit}>
          Add password
        </button>
      </form>
    </>
  ...
  const keysContent = (
    <>
      <form className="popup-form">
        ...
        <button
          className="popup-btn popup-keys-btn"
          type="button"
          onClick={(e) => {
            keyHandleSubmit(e);
          }}
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-buttons">
        <button className="new-password-btn" onClick={togglePopup}>
          <BsPlusCircleFill className="plus-icon" />
          New password
        </button>
        <button className="new-key-btn" onClick={toggleKeysPopup}>
          <BsPlusCircleFill className="plus-icon" />
          New key
        </button>
      </div>
      ...
      {isOpen && <Popup content={passwordsContent} handleClose={togglePopup} />}
      {isKeysOpen && (
        <KeysPopup content={keysContent} handleClose={toggleKeysPopup} />
      )}
    </div>
  );
}
